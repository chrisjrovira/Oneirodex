...
| Path | What it is |
|---|---|
| `oneirodex/` | Backend package. Routes split by surface: `routes_member.py`, `routes_admin_ext/`, `routes_apis/`. |
...
| `frontend/api-client` | `@oneirodex/api-client` typed fetch client — consumed by `clients/desktop` only. The SPAs use their own `src/api/` layers. |
| `clients/desktop` | Tauri companion. `clients/quest` is the VR client. |
| `docs/` | Start at [docs/README.md](docs/README.md). |

## Running tests

Postgres container: **`oneirodex-review-db`**. Start it if down (`docker start oneirodex-review-db`). Scoped pytest; `TEST_DATABASE_URL` must contain `test`. First-time setup: [docs/runbooks/local-postgres-pytest.md](docs/runbooks/local-postgres-pytest.md).
...
Return JSON through `api_ok` / `api_error` from `oneirodex/utils/api_response.py`. Pick `error_code` from `ERROR_CODES`. `detail` is passed to the browser as given — no secrets, tokens, raw `.env`, or filesystem paths.
