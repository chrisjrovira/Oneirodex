...
### 🧩 Optional modules
- 📡 *arr + hardlink pipeline · 🤖 Ollama AI assist · 🥽 **VR — any SteamVR / PSVR2-on-PC / Quest seat**: a headset-friendly `/vr` hub, honest *Plays in VR / community profile / plays flat* rows and a per-title headset record ([controllers-and-vr.md](docs/user/controllers-and-vr.md)); the runtime, adapter and streamer stay yours — [vr-byo-runtime.md](docs/runbooks/vr-byo-runtime.md) says which does what · 🔐 OIDC / Authentik SSO (opt-in)
