"""
Unit tests for sharewarez.routes_games_ext.add
...
from sharewarez.models import User, Game, Library, LibraryPlatform, UnmatchedFolder
...
class TestLinkExistingGame:
    ...
    def test_unsafe_path_rejected(self, client, admin_user, existing_game):
        _login(client, admin_user)
        with patch('sharewarez.routes_games_ext.add.is_safe_path', return_value=(False, 'Access denied')):
            with patch('sharewarez.routes_games_ext.add.get_allowed_base_directories', return_value=['/allowed']):
                response = client.post('/link_existing_game', data={
    ...
    def test_no_allowed_bases_configured(self, client, admin_user, existing_game):
        _login(client, admin_user)
        with patch('sharewarez.routes_games_ext.add.get_allowed_base_directories', return_value=[]):
            response = client.post('/link_existing_game', data={
                'game_uuid': existing_game.uuid,
                'full_disk_path': '/allowed/games/new-disk-folder',
            }, follow_redirects=False)
    ...
    def test_nonexistent_game_redirects_back(self, client, admin_user):
        _login(client, admin_user)
        with patch('sharewarez.routes_games_ext.add.is_safe_path', return_value=(True, None)):
            with patch('sharewarez.routes_games_ext.add.get_allowed_base_directories', return_value=['/allowed']):
                response = client.post('/link_existing_game', data={
                    'game_uuid': str(uuid4()),
                    'full_disk_path': '/allowed/games/new-disk-folder',
    ...
    def test_success_logs_system_event(self, client, admin_user, existing_game):
        ...
        with patch('sharewarez.routes_games_ext.add.is_safe_path', return_value=(True, None)):
            with patch('sharewarez.routes_games_ext.add.get_allowed_base_directories', return_value=['/allowed']):
                with patch('sharewarez.routes_games_ext.add.log_system_event') as mock_log:
                    response = client.post('/link_existing_game', data={
                        'game_uuid': existing_game.uuid,
                        'full_disk_path': '/allowed/games/new-disk-folder',
                    }, follow_redirects=False)
