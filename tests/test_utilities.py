...
class TestScanAndAddGamesCore:
    ...
    @patch('oneirodex.utilities.is_scan_job_running')
    ...
    def test_library_not_found_creates_no_scan_job(self, app, db_session):
        ...
        with app.app_context():
            with patch('oneirodex.utilities.is_scan_job_running', return_value=False):
        ...
        with app.app_context():
            with app.test_request_context():
                ...
                with patch('oneirodex.utilities.flash') as mock_flash:
                    with patch('oneirodex.utilities.redirect') as mock_redirect:
                        with patch('oneirodex.utilities.url_for') as mock_url_for:
                            mock_session = {}
                            ...
                            with patch('oneirodex.utilities.session', mock_session):
                                with patch(
                                    ...
                                    with patch(
                                        ...
                                        with patch('oneirodex.utilities.os.path.exists', return_value=True):
                                            with patch('oneirodex.utilities.os.access', return_value=True):
                                                with patch.dict(
    ...
    def test_form_validation_failure(self, app):
        ...
        with app.app_context():
            with app.test_request_context():
                ...
                with patch('oneirodex.utilities.session', mock_session):
                    with patch('oneirodex.utilities.flash') as mock_flash:
                        with patch('oneirodex.utilities.redirect'):
                            with patch('oneirodex.utilities.url_for'):
            ...
            with patch('oneirodex.utilities.is_scan_job_running', return_value=False):
                with patch('os.path.exists') as mock_exists:
                    with patch('os.access', return_value=True):
                        ...
                        with patch('oneirodex.utilities.get_game_names_from_folder', return_value=dummy_games):
                            with patch('oneirodex.utilities.load_scanning_filter_patterns', return_value=([], [])):
                                with patch('oneirodex.utils.scanning.process_game_with_fallback', return_value=True):  # Mock successful processing
                                    with patch('oneirodex.utilities.remove_from_lib') as mock_remove:
        ...
        with app.app_context():
            ...
            with patch('oneirodex.utilities.is_scan_job_running', return_value=False):
                with patch('os.path.exists', return_value=True):
                    with patch('os.access', return_value=True):
                        with patch('oneirodex.utilities.db.session.commit') as mock_commit:
            ...
            with patch('oneirodex.utilities.is_scan_job_running', return_value=False):
                with patch('os.path.exists', return_value=True):
                    with patch('os.access', return_value=True):
                        with patch('oneirodex.utilities.get_game_names_from_folder', return_value=[]):
                            with patch('oneirodex.utilities.load_scanning_filter_patterns', return_value=([], [])):
                                with patch('oneirodex.utilities.db.session.commit') as mock_commit:
                                    from sqlalchemy.exc import SQLAlchemyError
