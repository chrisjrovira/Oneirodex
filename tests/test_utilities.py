...
from gametheca.models import Library, GlobalSettings, AllowedFileType, ScanJob, Game
from gametheca.platform import LibraryPlatform
from gametheca.utilities import scan_and_add_games, handle_auto_scan, handle_manual_scan
...
class TestScanAndAddGamesCore:
    ...
    @patch('gametheca.utilities.is_scan_job_running')
    ...
    def test_library_not_found_creates_no_scan_job(self, app, db_session):
        ...
        with app.app_context():
            with patch('gametheca.utilities.is_scan_job_running', return_value=False):
            ...
            with patch('gametheca.utilities.is_scan_job_running', return_value=False):
                with patch('os.path.exists') as mock_exists:
                    with patch('os.access', return_value=True):
                        ...
                        with patch('gametheca.utilities.get_game_names_from_folder', return_value=dummy_games):
                            with patch('gametheca.utilities.load_scanning_filter_patterns', return_value=([], [])):
                                with patch('gametheca.utils.scanning.process_game_with_fallback', return_value=True):  # Mock successful processing
                                    with patch('gametheca.utilities.remove_from_lib') as mock_remove:
        ...
        with app.app_context():
            ...
            with patch('gametheca.utilities.is_scan_job_running', return_value=False):
                with patch('os.path.exists', return_value=True):
                    with patch('os.access', return_value=True):
                        with patch('gametheca.utilities.db.session.commit') as mock_commit:
            ...
            with patch('gametheca.utilities.is_scan_job_running', return_value=False):
                with patch('os.path.exists', return_value=True):
                    with patch('os.access', return_value=True):
                        with patch('gametheca.utilities.get_game_names_from_folder', return_value=[]):
                            with patch('gametheca.utilities.load_scanning_filter_patterns', return_value=([], [])):
                                with patch('gametheca.utilities.db.session.commit') as mock_commit:
                                    from sqlalchemy.exc import SQLAlchemyError
