...
from gametheca.utils.status import (
    get_system_info,
    get_config_values,
    get_active_users,
    get_log_info,
...
class TestGetSystemInfo:
    ...
    @patch('gametheca.utils.status.socket.gethostbyname')
    @patch('gametheca.utils.status.socket.gethostname')
    @patch('gametheca.utils.status.platform.python_version')
    @patch('gametheca.utils.status.platform.version')
    @patch('gametheca.utils.status.platform.system')
...
class TestGetConfigValues:
    ...
    @patch('gametheca.utils.status.os.path.exists')
    @patch('gametheca.utils.status.os.access')
    def test_get_config_values_with_existing_paths(self, mock_access, mock_exists):
        ...
        with patch('gametheca.utils.status.Config') as mock_config:
            mock_config.DATA_FOLDER_GAMES = '/test/games/path'
            mock_config.IMAGE_SAVE_PATH = '/test/images/path'
            mock_config.UPLOAD_FOLDER = '/test/uploads/path'
            
            result = get_config_values()
            
            expected_keys = ['DATA_FOLDER_GAMES', 'IMAGE_SAVE_PATH', 'UPLOAD_FOLDER']
    ...
    def test_get_config_values_with_nonexistent_paths(self, mock_access, mock_exists):
        ...
        # Mock Config class with test paths
        with patch('gametheca.utils.status.Config') as mock_config:
            mock_config.DATA_FOLDER_GAMES = '/nonexistent/path'
            
            result = get_config_values()
            
            if 'DATA_FOLDER_GAMES' in result:
                assert result['DATA_FOLDER_GAMES']['exists'] is False
                assert result['DATA_FOLDER_GAMES']['read'] is False
    ...
    def test_get_config_values_mixed_permissions(self, mock_access, mock_exists):
        ...
        with patch('gametheca.utils.status.Config') as mock_config:
            mock_config.DATA_FOLDER_GAMES = '/readonly/path'
            
            result = get_config_values()
            
            if 'DATA_FOLDER_GAMES' in result:
                assert result['DATA_FOLDER_GAMES']['exists'] is True
