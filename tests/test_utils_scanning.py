...
from oneirodex.utils.scanning import (
...
class TestTryAddGame:
    ...
    @patch('oneirodex.utils.game_core.retrieve_and_save_game')
    ...
    @patch('oneirodex.utils.game_core.retrieve_and_save_game')
    def test_try_add_game_blocks_heuristic_name(
        self, mock_retrieve, db_session, sample_library, sample_scan_job, monkeypatch,
    ):
        monkeypatch.setenv('ENABLE_MALWARE_SCAN', 'true')
        monkeypatch.setenv('MALWARE_SCAN_BLOCK_ON_HIT', 'true')
        result = try_add_game(
            'CoolGame.iso.exe',
            '/storage/CoolGame.iso.exe',
            sample_scan_job.id,
            sample_library.uuid,
        )
        assert result is False
...
class TestProcessGameWithFallback:
    ...
    @patch('oneirodex.utils.scanning.try_add_game')
    ...
    @patch('oneirodex.utils.scanning.try_add_game')
    @patch('oneirodex.utils.scanning.log_unmatched_folder')
...
class TestProcessGameUpdates:
    ...
    def test_process_game_updates_success(self, mock_isdir, 
        ...
        with patch('os.listdir', side_effect=listdir_side_effect), \
             patch('oneirodex.utils.scanning.read_first_nfo_content', return_value='Test NFO content'):
...
class TestRefreshImagesInBackground:
    ...
    @patch('oneirodex.utils.scanning.delete_game_images')
    @patch('oneirodex.utils.game_core.process_and_save_image')
    @patch('oneirodex.utils.scanning.make_igdb_api_request')
