...
from gametheca.utils.email_digest import run_email_digest_batch, send_digest_for_user
...
def test_digest_skips_when_pref_off(app, db_session, member):
    with app.app_context():
        ...
        with patch('gametheca.utils.email_digest.send_email_quiet') as send:
...
def test_digest_sends_and_sets_watermark(app, db_session, member):
    with app.app_context():
        ...
        with patch('gametheca.utils.email_digest.send_email_quiet', return_value=True) as send:
...
def test_run_batch_counts(app, db_session, member):
    with app.app_context():
        ...
        with patch('gametheca.utils.email_digest.send_email_quiet', return_value=True):
