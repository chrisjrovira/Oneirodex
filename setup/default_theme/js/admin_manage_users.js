document.addEventListener('DOMContentLoaded', function() {
    ...
    function syncChildControlsVisibility() {
        const isChild = document.getElementById('editRole')?.value === 'child';
        ['editLibraryAllowlistWrap', 'editDeniedGenresWrap', 'editDeniedThemesWrap'].forEach((id) => {
            const wrap = document.getElementById(id);
            if (wrap) {
                wrap.style.display = isChild ? '' : 'none';
            }
        });
    }

    document.getElementById('editRole')?.addEventListener('change', syncChildControlsVisibility);
    ...
    document.getElementById('saveUserChanges').addEventListener('click', function() {
        ...
        const userData = {
            ...
            allowed_library_uuids: selectedOptions('editAllowedLibraries'),
            denied_genres: selectedOptions('editDeniedGenres'),
            denied_themes: selectedOptions('editDeniedThemes'),
        };
    ...
    function fetchUserDetails(userId) {
        fetch(`/admin/api/user/${userId}`)
            ...
            .then(data => {
                ...
                setSelectedOptions('editAllowedLibraries', data.allowed_library_uuids || []);
                setSelectedOptions('editDeniedGenres', data.denied_genres || []);
                setSelectedOptions('editDeniedThemes', data.denied_themes || []);
                syncChildControlsVisibility();
                editModal.show();
            })
            .catch(error => {
                console.error('Error:', error);
