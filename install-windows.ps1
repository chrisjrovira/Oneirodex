<#
.SYNOPSIS
    GameTheca Windows Installer v1.0

.DESCRIPTION
    Sets up a native (non-Docker) GameTheca install on Windows: virtual
...
function Write-Header {
    ...
    Write-Host '    GameTheca Windows Installer v1.0' -ForegroundColor White
    Write-Host '===============================================' -ForegroundColor Cyan
    Write-Host ''
}
...
function New-Databases {
    if ($SkipDb) {
        ...
        return 'postgresql://postgres:postgres@localhost:5432/gametheca'
    }
...
function Write-EnvFile([string]$DatabaseUrl) {
    ...
    $uploadFolder = (Join-Path $ScriptDir 'gametheca\static\library')
    $devMode = if ($Dev) { 'true' } else { 'false' }

    $lines = @(
        "# GameTheca Configuration - Generated by install-windows.ps1 $(Get-Date)",
        '',
        '# Database connection',
        ...
        "DEV_MODE=$devMode",
        '',
        '# Local HTTP - set both true once GameTheca sits behind HTTPS',
...
function Show-Summary {
    ...
    $answer = Read-Host 'Start GameTheca now? [Y/n]'
    if ([string]::IsNullOrWhiteSpace($answer) -or $answer -match '^[Yy]') {
        Write-Note "Starting GameTheca - open http://localhost:$Port"
        & (Join-Path $ScriptDir 'startweb_windows.cmd')
    }
    else {
        Write-Note 'To start GameTheca later, run: .\startweb_windows.cmd'
    }
}

# ---------------------------------------------------------------- main

"GameTheca Windows Installer - $(Get-Date)" | Set-Content -Path $LogFile -Encoding utf8
Write-Header

Write-Note 'This installer will:'
Write-Note '  - Check for Python and PostgreSQL 17'
Write-Note '  - Create the gametheca databases'
