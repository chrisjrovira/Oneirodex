...
Identify UI today searches **Steam · RAWG · GOG** (`SUPPORTED_SOURCES` in `metadata_search.py`). Artwork providers are IGDB / SteamGridDB / GiantBomb (`providers/`). Ownership sync accepts **steam | gog | epic | amazon** (`VALID_STORES` in `store_ownership.py`). Steam / GOG / Epic / Amazon are **live** register sync when a credential is saved (unofficial Galaxy / launcher / Nile surfaces). CSV still works.
...
| **P2** | **Amazon / Prime Gaming** | Low | Live register via unofficial Nile/Heroic + CSV (already in `VALID_STORES`) | Ownership live sync shipped 2026-08-27; identify search later |
...
| **Amazon / Prime Gaming** | **No** public official library API. Nile / Heroic use unofficial entitlements. | **Ownership:** unofficial Nile/Heroic live register + CSV. Identify search later. Never a download. |
