...
| Collection detail | — | Delete collection, with the collection's name as the summary. **This one was a live bug**: the retirement rule matches `.gt-page-header > h1`, and here that h1 is the collection's name — so under v2 the page rendered with nothing at all identifying which collection you were in |
| Activity | Everyone · Friends only | The "friends only feed" checkbox was narrowing *which activity you are looking at* — a sibling view, not a page setting — so it becomes the segmented control |
| Trailers | — | Settings · Another one, plus Exit Attract Mode · Big Picture while attract mode is on. Its `h1` was never page identity — it names the trailer *now playing* and links to that game, so it becomes the summary and stays a link rather than being retired as a title |
