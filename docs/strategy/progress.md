...
| PS1 browser play (cue+bin zip delivery) | **Shipped, uncommitted** — `.cue` + sibling `.bin`/`.img`/`.raw`/`.wav` bundled as stored `play.zip` (`bundle_playable_rom_zip` · ASGI `/api/downloadrom`) · WebRetro reads `Content-Disposition` filename · 300s ROM fetch timeout · `.pbp`/`.img`/`.raw`/`.wav` in `ROM_EXTENSIONS` · **SCPH BIOS still required** — [browser-play.md](../user/browser-play.md#ps1-and-other-disccue-downloads-are-bundled-as-a-zip) · [troubleshooting.md](../user/troubleshooting.md) |
