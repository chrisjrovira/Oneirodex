...
**Release:** **0.2.0** (ship imminent — Jul 29 META/IMG/ART + Wave 1+2 + process refresh; QA **138/138 PASS**)  
**Updated:** 2026-07-29 (**META-1 / IMG-1 / ART-5 + process + Wave 1+2 on ship path** — Wave 1+2 feedback fixes: BASE_FOLDER RO exempt · Docker bsdtar/7z for `.rar` · redundant page H1s removed · favorite moved top-right (stacks under menu, badges keep top-left clean) · smoother fractional tile slider · admin pages full-width (1600px) · scan-mgmt unmatched copy/open/export + fix-search · image queue preview/group/retry (+ eager-download bugfix) · art studio disk-error JSON + orphan rollback · Discover custom zones (manual/filter) · play aspect-lock + expanded room art (32X/Saturn/PS2/GameCube/NGC ambience, NDS clamshell width) · SNES A/V tuning (audio latency/skew, vsync, overclock-cycles option) · slim/hidden-select theme picker · profile pages suppress legacy LHN · **PS1 browser play:** cue+bin disc sets bundled as `play.zip` via `rom_archive.bundle_playable_rom_zip` + ASGI `/api/downloadrom` · WebRetro `base.js` reads `Content-Disposition` filename + 300s ROM fetch timeout · `.pbp`/`.img`/`.raw`/`.wav` in `ROM_EXTENSIONS` — **META-1 / IMG-1 / ART-5 shipped (shipping)** (Meta Quest identify+CSV · cover selection · art system templates · fishermans ≥4-token colon) — **QA 138/138 PASS** — **next:** human ship-ready (commit+push) → Unraid free disk (~99%) → rebuild → `IMAGE_SAVE_PATH` RW → Reset Themes → leaf libs · **PS1 still needs SCPH BIOS** · Chat capture skipped (login 500 / no new member UI hero))
...
| IGDB name-resolution strategy | **Shipped (docs)** — letter-bucket / BGDA matcher rules · heuristic colon ≥4 tokens (fishermans) — [name-resolution.md](name-resolution.md) |
| IGDB name variants (matcher) | **Shipped (code)** — BGDA1 → Baldur's Gate: Dark Alliance · known-subtitle before ≥4-token heuristic · Unraid `_pc`/`_b` rescan next (code on main @ b3db749d) — [name-resolution.md](name-resolution.md) |
...
| Full-app 3-pass review Jul 28 | **Complete** — P1–P3 + Pass A–F + Tokens UI + play polish on main. Wave 1+2 + PS1 + META/IMG/ART on ship path. **Next:** human Unraid rebuild + Reset Themes + free disk → leaf libs — **Blocked:** host disk ~99% · Chat capture (login 500) · PS1 SCPH BIOS — see program canvas |
...
| IMG-1 Cover selection + image queue | **Shipped (shipping)** — single+mass cover search/apply APIs · image queue `failure_reason` + `image_save_path` · admin **ImagesPage** / **ArtworkPicker** · Auto-pick / mass queue — [libraries-and-scans.md](../admin/libraries-and-scans.md) · [settings-modules.md](../admin/settings-modules.md) |
...
| README live media | **On branch `cursor/readme-live-screenshots` @ 7ef42a94 (local, not pushed)** — hero · Library · Systems live PNGs; mock JPGs deleted — [CAPTURE.md](../assets/readme/CAPTURE.md) · **Chat blocked** (`screenshot-chat.png`) — Capture **skipped** this turn (login 500 / no new member UI hero); Docs re-captures when app healthy · **next:** Human merge/push → main |
