...
| **UX-A6** | Remove every **sharewarez** mention and leftover code | **Working tree already clean** (0 hits, incl. variants + tracked filenames). Remaining: the **initial commit message** "SharewareZ rewrite" — needs a history rewrite (SCRUB-5, deferred); force-push breaks clones, so **human call** |
