...
| **UX-B6** | Loading icons: **animated per icon**, shown as a **popup** — inline spinners shift the layout on every server event — **Done 2026-08-27.** `LoadingOverlay` on library boot (blocking), Ownership (delay 0), and Trailers **Another one** (250ms, previous player stays). Remaining page-owning loads and leftover action/section errors use `PageStatus`. Button `gt-spinner--sm`, DiscoverShelf's reserved pending tile, SpaceRail **Loading spaces…**, Tokens clipboard copy, and Set completion’s missing-set empty stay inline on purpose |
