...
- `POST /api/updates/scan` (W28) — the **member's own** re-probe. The inbox above only reads what a previous probe found, so a library that had never been probed showed an empty inbox with no member-reachable way to fill it. One bounded batch per call (`limit` default 25, max 50), accessible titles only, oldest `freshness_checked_at` first, `only_stale` skipping anything probed inside 24h. Returns `checked` / `behind[]` / `errors[]` and `remaining`, so the page can say how much of the library is still unswept instead of implying one press did all of it. Bounded because each title is a live store request.
...
- **UI:** Calendar **List / Month** (Agenda retired W28 — it was List with week headings) with per-day cover art that cycles every 10s when several titles land on a day · Updates auto-refresh (~50s while tab visible) + **Refresh** and its timestamp on the inbox heading row + **Check library for updates** + calendar teaser · Library sticky wishlist + play status (Wave 11) — [faq.md](../user/faq.md#updates--calendar)
