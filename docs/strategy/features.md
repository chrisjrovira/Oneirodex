...
- BYO acquire (Prowlarr/Jackett + qBit + optional Real-Debrid/AllDebrid) — not bundled pirate indexers
...
**Still out of scope:** pirate indexer hosting, DRM store download/install queues, multiplayer cheat injection.
...
**Goal:** Optional *arr-style acquisition*. Feature-flagged; off by default.
...
**Goal:** One Game, many `GameVersion` (repack/scene tags, GOG, Update packs) with selector on details + download.
...
| 20c | **Challenge / captcha bypass** | BYO TRAWL / FlareSolverr-compat + token APIs — [challenge-bypass.md](challenge-bypass.md); opt-in; wire acquire |
