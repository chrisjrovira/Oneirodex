...
**Owner:** Game Master (rules) → Backend (A0–A14, B15–B20, BE-DET-1…10 shipped)  
**Related:** [libraries-and-scans.md](../admin/libraries-and-scans.md) · [store-metadata-identify.md](store-metadata-identify.md) · archived design [game-recognition-and-rename-design.md](archive/superpowers-2026-07/specs/2026-07-22-game-recognition-and-rename-design.md)
...
- [x] **BE-DET-5** — multi-disc grouping · `(Disc|Disk|CD N)` peel → one Game + `GameExtra(extra_kind='disc')` · cue+bin companion filter · `disc_index`/`disc_count` · browse/details `is_multi_disc`/`discs[]` · threshold **0.92** · **QA PASS 119/119** · DoD met · live skipped · details disc chips shipped 2026-08-27 (not on tiles)
