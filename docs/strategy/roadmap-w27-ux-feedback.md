...
| ID | Item | Notes |
|---|---|---|
| **W27-C1** | **Every table must match** in theming and style, and so must **every category of button** | The unfinished half of UX-C8: `DataTable` exists, four surfaces still hand-rolled |
| **W27-C2** | **Active scan jobs** table not sortable by clicking column headers — as every table should be | |
