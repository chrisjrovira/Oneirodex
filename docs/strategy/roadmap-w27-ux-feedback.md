...
| ID | Item | Notes |
|---|---|---|
| **W27-C1** | **Every table must match** in theming and style, and so must **every category of button** | **Advanced, not closed.** The W26 note ("four surfaces still hand-rolled") was stale: `OpsPage`, `pages.jsx`, `ProposeLeafLibraries` and `ImportLeafLibraries` all use `DataTable` now. The real remainder was that `DataTable` is React, leaving every classic Jinja table outside the contract — addressed by `gt_sortable_table.js` (see C2), whose `.gt-sort-btn` rules live in the shared `table-components.css` rather than beside one page. The unmatched table has since adopted it: its hand-written sort buttons, its own sorter, its indicator bookkeeping and its `.unmatched-sort-btn` rules are all gone, and the module grew `data-gt-sort-default` — the counterpart to `DataTable`'s `initialSort` — so that table still arrives folder-ascending without a page script re-sorting after every render. The Ops and dashboard panels followed: of seven raw `gt-ops-table` blocks, five are now `DataTable` and **two are deliberate keeps** — `DetailPanel` (a key/value block with no header row) and Services (a fixed diagnostic checklist whose row order is what an operator scans, with bespoke cells rather than one shape repeated). Both now say so in a comment, so they read as decisions. Migrating them needed `toolbar={false}`: these panels hold three or four rows, and the filter box is what had made hand-rolling them look reasonable — the styling was already shared, so the migration is visually a no-op that adds sorting and the shared empty state. **Buttons, partly.** The two families are `.gt-btn` (primitive) and `.gt-cbtn` (chrome, shared by both SPAs and the Jinja partials). They disagreed on behaviour, not just looks: `.gt-cbtn` had **no disabled state**, so three member pages rendered disabled buttons that looked and hovered like live ones, and Ops had patched the gap for itself alone. Fixed at the shared class, local copy deleted, focus treatment unified across both families with a fallback so a theme missing `--gt-focus-ring` loses the colour rather than the whole outline. **Still open:** `.gt-cbtn` remains off the token scales — its raw padding/type/radius map to no existing step, so tokenising changes rendered chrome size on every page and wants someone looking at the running product. The long tail (`.favorite-btn`, `.game-status-btn`, `.filter-btn`, `.scan-btn`, `.browse-btn`, `.control-btn`, `.up-btn`, `.create-library-btn`, `.gt-themes-btn`, `.gt-action-bar__btn`) is untouched and is the larger half of "every category of button" |
...
| **W27-C4** | **Unmatched table redesign** — easier to interact with, plus a **preview pane for dupes that pops out** so the detail is readable | **Pop-out done 2026-08-26** on the live Jinja unmatched table (`Pop out` opens a wide dialog of the same two-column compare). Inline compare stays. Per-theme Reset Themes for `admin_manage_scanjobs.js` / `.css`. Broader table redesign (density of the whole unmatched page) is still open if it still reads cramped after the pop-out |
...
| ID | Item | Notes |
|---|---|---|
| **W27-D1** | **Ops and Server info are still two pages** and must be one | **Done.** GT-B21's claim was half true — Ops *did* already render System / Database / Logs; what survived was the page itself. `/admin/new_server_info` is now retired at every layer (route, template, rail link, ops-glance deep link, route resolver, icon test). The one thing only that page showed — **config values** — was added to `/admin/api/ops/system` and rendered as a fourth Ops panel **first**, because retiring a page that still holds the only copy of something is how a merge loses information. The three duplicated key/value panels were collapsed into one `DetailPanel` rather than becoming four. **2026-08-26:** `/admin/server_status_page` is a redirect to `/admin/ops` (login + admin still required). The template remains on disk for the icon test; the live suite no longer renders it |
...
| **W27-D3** | **Statistics needs a full redesign** — large amounts of empty space, and odd scrolling on both axes | **Layout done 2026-08-26.** Bootstrap `.row` / `.col-md-6` plus Chart.js `responsive` without `maintainAspectRatio: false` let canvases grow unbounded. Charts now sit in `repeat(auto-fit, minmax(min(100%, 28rem), 1fr))` with a fixed 17.5rem box. Headline strip and titles table from UX-C13 stay. Not a visual redesign of the chart skins |
...
| **W27-E4** | **All LHN icons should follow the selected theme's colour** — supply alternates for each icon in each theme | **Code half done 2026-08-26.** Resting rail glyphs use `color: var(--gt-accent)` so they inherit the preset; labels stay muted so the active row is still distinct. **Still open — art:** per-theme alternate drawings (6 packs × rail glyphs). Do not imitate manufacturer trade dress |
