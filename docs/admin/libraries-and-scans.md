...
- **UI:** When a scan is already busy, Scan management opens a conflict modal — **Queue this scan** (default focus) or **Force run now (parallel)** with an Unraid/NAS load warning — on **Auto Scan**, **Manual Scan** (busy path only; idle Manual stays **List Games** identify), Refresh all, and restart-while-busy (Admin SPA Libraries / Scans also offer Refresh all + the same modal). Flask flashes Backend `message` (includes position / coalesce). JSON surfaces (Refresh all) toast `queued` / `started` / `rejected` (`Queued · position N`, optional `· coalesced`). After theme CSS/JS updates, **Admin → Themes → Reset Themes** so `library/themes` picks up `admin_manage_scanjobs` changes.
...
- **Drain reliability:** when a job leaves Running/Stopping (Complete / Fail / Cancel), Backend promotes the oldest Queued job. Safety nets also run on the **scan scheduler** (60s), **Ops summary** poll, and **`GET /api/scan_jobs_status`** — reclaim stale Stopping (~10m) / Running-without-progress (~6h), then promote if idle. Worker early-exit / crash paths mark Failed so the queue cannot stick forever.
...
**Wave 17 UI (classic scanjobs + Dupe glance):** filters toolbar (status · search · Why chips · suggested-kind chips), row checkboxes + sticky batch bar, inline **Amend naming** (label exact; secondary **On disk: {basename}**; soft `search_name` / optional `display_name` — never disk rename), and **Dupe of …** on the **base table** (thumb · name · uuid · path · match_score) so operators see the library hit without opening Dupe glance. Disk tidy this wave = **Open path** only. Soft-degrades when batch/name/`matched_game` endpoints are mid-rollout (client filter + `/duplicates` enrich + per-id fan-out). After CSS/JS deploy: **Admin → Themes → Reset Themes**.

**W20-7 UI (unmatched triage filters):** Admin → Scan management → **Unmatched Folders** adds **Leaf** chips (**File leaf** / **Folder leaf** — ROM/archive file vs named folder; client heuristic in `unmatchedTriage.js`) and **Triage** chips (**Platform mismatch** — path platform hint disagrees with library assignment · **Garbage** — installers/redistributables/temp scaffolding). Each row shows matching badges in the **Folder** column (under **Amend**). Sticky batch bar adds **Ignore selected** — tries `POST /api/unmatched_folders/batch/ignore` first, then falls back to per-id `toggle_ignore_status` when batch is unavailable. After deploy: **Admin → Themes → Reset Themes** (or hard-refresh) so `library/themes` copies pick up `admin_manage_scanjobs` CSS/JS/HTML and `unmatchedTriage.js`.
...
- **Fix search / Identify as game** — opens manual add/identify with the folder's basename cleaned (release-group tags, dots/underscores, scanning filter patterns stripped) and prefilled as the search query — same cleanup the auto-matcher uses. When a soft **`search_name`** is set on the Unmatched row (Wave 17), Fix/identify prefer that over the cleaned basename. Soft amend never renames the on-disk folder.
- **Amend naming (soft)** — `PATCH`/`POST /api/unmatched_folders/<id>/name` `{ search_name?, display_name?, name? }` (`name` aliases `search_name`). Stores librarian-facing search/display strings only; `folder_name` stays the derived basename; `folder_path` unchanged. Batch: `POST /api/unmatched_folders/batch/amend`.
- **Mark as Experience / Emulator / Tool** — `POST /api/unmatched_folders/<id>/mark_kind` catalogs the folder as gaming software (custom-range Game + `item_kind`) **without** inventing an IGDB Main Game match. Success/error toasts are honest. Also available on Admin Dashboard **Dupe glance** (filter Unmatched). Prefers soft `search_name` when set.
...
**Duplicate glance + fix (Wave 2a shipped; Wave 17 Dupe of on base table):** Admin Scan management / Dashboard **Dupe glance** shows Duplicate / Unmatched rows with Open path, Fix search, Mark as Experience/Emulator/Tool, Merge/Keep/Ignore, Fix false duplicates, and the same **Dupe of …** library hit as the Scan management base table. Wave 17 list/export nest `matched_game` on Duplicate (or `matched_game_uuid`) rows so the base Unmatched table can show the dupe target without opening Dupe glance.
...
- **TheGamesDB (W20-5c + W21 Stage E):** optional `THEGAMESDB_API_KEY` (or `GlobalSettings.thegamesdb_api_key`) — empty results when unset (honest, no 500). Manual identify / covers + **Stage E propose-only** platform-filtered exact-title hints on console leaves after Stage D miss (no Game create).
...
Folder → IGDB matching cleans the folder basename in Stage **A0–A14** (`parse_game_label`: scene/repack brackets + unbracketed suffixes, Incl Update / Update·Build prose, date-stamps, version/build/VR/EA junk, Steam App ID, edition/add-on peel, franchise apostrophe inject, VR re-pass after version), then expands search variants (colon subtitles, trailing bare `1` strip, `2`/`II` sequel swaps, year/pack/edition peel, Steam title as query #0 when App ID resolves, C11 bare-franchise propose-only). **Done (uncommitted, QA 166 PASS)** — details: [name-resolution.md](../strategy/name-resolution.md). Peel aggressiveness + score thresholds are operator-tunable — [Scan / match policy (W20-4)](#scan--match-policy-w20-4).

**Stage D (W20-5a, uncommitted + W21 App-ID harden):** After IGDB high-confidence miss (and not C11 / propose-only), scan tries Steam App ID **only when store details succeed and the folder title corroborates** the Steam name (exact or remaster/subtitle primary prefix); otherwise fall through to **exact** Steam/GOG storesearch title — custom `igdb_id ≥ 2000000420`, register-only store links, no fuzzy auto-import. Wrong-namespace folder `(digits)` no longer stamp a bogus `steam_app_id` on details miss. Ambiguous or miss → Unmatched + software proposal as today. See [store-metadata-identify.md](../strategy/store-metadata-identify.md#stage-d--igdb-miss--steamgog-custom-w20-5a--done-uncommitted).

**Stage E (W21-BE-2, uncommitted):** After Stage D miss → propose-only MobyGames (PC) / platform-filtered TheGamesDB (console) exact-title hints on proposal sidecar + `suggested_candidate_name` — **no** Game create. Keys optional (skip silently when unset). **DAT unique-hash (W21-BE-DAT):** after Stage D miss, unique CRC/MD5/SHA1 vs uploaded reference sets may auto-create a custom Game before Stage E; ambiguous/missing DAT stays Unmatched. See [store-metadata-identify.md](../strategy/store-metadata-identify.md#stage-e--moby--thegamesdb-after-stage-d-propose-only--done-uncommitted-w21-be-2).

**MobyGames (W20-5b, uncommitted):** Manual identify source (`source=mobygames|moby`) + Stage E propose-only. Optional `MOBYGAMES_API_KEY` — empty results when unset. Not Stage D auto-import. **Ops:** app restart for `mobygames_api_key` · set key for live hits. See [store-metadata-identify.md](../strategy/store-metadata-identify.md#w20-5b--mobygames-manual-search-done-uncommitted).

**TheGamesDB (W20-5c, uncommitted):** Manual identify source (`source=thegamesdb|tgdb`) + Stage E propose-only on console. Optional `THEGAMESDB_API_KEY` — empty results when unset. Not Stage D auto-import. **Ops:** app restart for `thegamesdb_api_key` · set key · **Reset Themes** for identify JS. See [store-metadata-identify.md](../strategy/store-metadata-identify.md#w20-5c--thegamesdb-manual-search-done-uncommitted).
