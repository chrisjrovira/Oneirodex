...
- **Scan Jobs redesign:** LHN **Scan Jobs** is the full jobs table (When column, clear completed/failed/all, CSV export). Auto/Manual keep a compact **Scan jobs** modal. Schedules support Once, 8/24/48h presets, every N minutes/hours, and 5-field cron. Scan mode includes **Auto** (folders vs files heuristic). After theme asset deploys: **Admin → Themes → Reset Themes**.
