...
**Libraries & scans** is one document with eight in-page panes (Libraries / Auto / Tools / Manual / Unmatched / Filters / Extensions / Image queue). The two-bar context strip matches the member catalog; switching a pane does **not** load a new page. Each pane is one primary list or form on the page shell — not a card wrapping a table wrapping another card.
...
- **UI:** When a scan is already busy, Scan management opens a conflict modal — **Queue this scan** (default focus) or **Force run now (parallel)** with an Unraid/NAS load warning — on **Auto Scan**, **Manual Scan** (busy path only; idle Manual stays **List Games** identify), Refresh all, and restart-while-busy (Admin SPA Libraries / Scans also offer Refresh all + the same modal). Flask flashes Backend `message` (includes position / coalesce). JSON surfaces (Refresh all) toast `queued` / `started` / `rejected` (`Queued · position N`, optional `· coalesced`). Classic Scan management skips overlapping 3s job polls and hidden-tab ticks (same for the 30s unmatched poll); the React Scans / Ops / Dashboard polls do the same. After theme CSS/JS updates, **Admin → Themes → Reset Themes** so `library/themes` picks up `admin_manage_scanjobs` changes.
...
**Stage D (W20-5a, uncommitted + W21 App-ID harden):** After IGDB high-confidence miss (and not C11 / propose-only), scan tries Steam App ID **only when store details succeed and the folder title corroborates** the Steam name (exact or remaster/subtitle primary prefix); otherwise fall through to **exact** Steam/GOG storesearch title — custom `igdb_id ≥ 2000000420`, register-only store links, no fuzzy auto-import. Wrong-namespace folder `(digits)` no longer stamp a bogus `steam_app_id` on details miss. Ambiguous or miss → Unmatched + software proposal as today.
...
**Empty shelves (systems with no dumps yet):** Systems tiles come from Library rows, not from the enum list. A leaf with 0 games still shows. Household placeholders (VB / Wii / Wii U / 3DS / Pokémon Mini / Vita / Xbox siblings / Commodore 8-bit / CD-i / Pico / Jaguar CD / Amiga CD32 / MSX / ZX Spectrum / Amstrad CPC / Atari ST / Apple II / Atari 8-bit / X68000 / PC-98) are on the games share as of 2026-08-29. Import [empty-shelf-import.csv](../strategy/empty-shelf-import.csv) via **Add many: import a list**, then confirm create (never auto). Propose-from-tree is the secondary path. Do not point at a missing path and hope scan invents files. Full folder table + remaining enum-gap list: [console-gaming-libraries.md](../strategy/console-gaming-libraries.md#empty-shelves-systems-you-do-not-hold-yet). Oneirodex never fetches ROMs.
