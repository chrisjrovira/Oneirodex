...
| Token pass on classic CSS (`form-components.css`, `sidebar.css`, `table-components.css`, settings shell, system logs, integrations tabs, whitelist, IGDB settings) | Theme tree CSS → **library volume** | After 2026-08-26 UID-017: **Reset Default Themes** so volume copies pick up `var(--gt-radius-*)` / `var(--gt-font-*)`. Member SPA pages (Help, Trailers, VR, Ownership, mobile density) ship in **`member-app` dist** — rebuild the image; Reset Themes does not refresh those |
