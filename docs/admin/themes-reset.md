...
| Scan management CSS/JS (`admin_manage_scanjobs.css` / `.js` — unmatched Open path, table scale, Actions wrap, **Scan Filters** how-it-works / quick-add / `dir:` kind UI, Wave 17 Amend/batch/Dupe-of, **Wave 18** scan job elapsed/ETA/stalled + status/library/path filters, **W20-2** Name transform trail `<details>`) | Same theme tree → **library volume** copies | Needs rebuild **and** **Reset Default Themes** (or hard-refresh alone is not enough if volume copies are stale) — without Reset, unmatched tab can look cramped / miss Open path chrome / miss Scan Filters explain UI / miss Wave 18 timing+filter chrome / miss W20-2 transform trail expander |
