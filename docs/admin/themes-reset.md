...
| Token pass on classic CSS (`form-components.css`, `sidebar.css`, `table-components.css`, settings shell, system logs, integrations tabs, whitelist, IGDB settings, then 2026-08-27 scan jobs / game details / base / account / setup) | Theme tree CSS → **library volume** | After UID-017: **Reset Default Themes** so volume copies pick up `var(--gt-radius-*)` / `var(--gt-font-*)`. Member SPA pages ship in **`member-app` dist** — rebuild the image; Reset Themes does not refresh those |
