...
> be changed. The ui should be overhauled to look different than sharewarez
> it still looks just like it. Perhaps see about changing the lhn to on top.
...
- **The IDE workspace root points at `C:\Users\cephyrix_zyth\Desktop\sharewarez-build`, which does not exist.** Glob and Grep hard-fail against it. Ask the user to reopen the folder as `gametheca`, or fall back to `Shell` with `required_permissions: ["all"]` and an explicit `working_directory`, plus `Read` with absolute paths.
