...
**Goal:** Improve IGDB matching for real Unraid library names (FitGirl/HV, Steam App IDs, versions), gate low-confidence imports behind review, then add template-based disk rename, proposal sidecars, and a library-doctor batch job.
...
def test_fitgirl_plus_versionish_left_readable():
...
_BRACKET_TAG_RE = re.compile(
    r'\[\s*(?:FitGirl(?:\s+HV)?|Dodi|CODEX|FLT|SKIDROW|EMPRESS|RUNE|PLAZA|HOODLUM)\s*(?:Repack)?\s*\]',
    re.IGNORECASE,
)
...
Wire `clean_game_name` to optionally use `parse_game_label` for bracket/App ID first, then existing pipeline on `cleaned_name` **without** re-stripping Remastered via the `Repack|Edition|Remastered|Remake` regex — change that line to strip `Repack|Proper|Dodi` only (keep Remastered/Remake/Edition for scoring). Update existing tests that expected Remastered removal.
...
- [ ] **Step 1: Failing tests** — exact match → ~1.0; FitGirl cleaned vs IGDB title high; unrelated low; two close scores → low
...
**Phase 1B exit:** temp-dir pytest; manual FitGirl folder rename; installed `E:\games\FFXV` guts unchanged.
