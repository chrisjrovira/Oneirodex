...
- Default theme must not clone generic teal-glass media-library chrome
...
- Replace: `gametheca/static/newstyle/gt_bg.jpg` (new), overwrite `default_cover.jpg` / `default_library.jpg` (keep local backups only if needed — do not commit backup files)
...
:root {
  ...
  --gt-accent: #ff5a36; /* arcade accent — not generic teal-glass */
