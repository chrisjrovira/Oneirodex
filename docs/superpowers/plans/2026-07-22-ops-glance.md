...
- Produces:
  - `derive_issues(*, disk_base_percent, disk_warez_percent, path_problems, scan_failures, recent_error_count) -> dict`
...
**Interfaces:**
- Consumes: `get_cpu_usage`, `get_memory_usage`, `get_disk_usage`, `get_warez_folder_usage`, `get_system_info`, `get_config_values`, uptime formatters, `get_network_stats`, `derive_issues`, SQLAlchemy models (`Library`, `Game`, `UnmatchedFolder`, `DownloadRequest`, `ScanJob`, `SystemEvents`)
