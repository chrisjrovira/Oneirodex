...
| Rules: `prompt-brief.mdc`, `docs-sync.mdc`, `pm-disperse.mdc` | Always on |
...
Attach with `@` (skills have `disable-model-invocation: true`). Matching rules: `.cursor/rules/agent-*.mdc`. Each seat SKILL includes **When to invoke**, **Task prompt**, and **End of turn**.

| Seat | Skill | Owns |
|---|---|---|
| PM | **agent-pm** | Backlog, sequencing, Task briefs (no product code) |
| 1 | **agent-uiux** | Member + admin SPA chrome, aurora theme |
| 2 | **agent-backend** | Flask/ASGI/APIs/schema/runtime |
...
| — | **agent-team** | Index + wave protocol + shared Task skeleton |

Typical wave: parent **PM** → optional GM/Ops consult → **Task** implementers parallel → Task **QA** → Task **Docs** (docs-sync + **Canvas: synced**). Parent does **not** land product code when seats exist (`pm-disperse.mdc`).
