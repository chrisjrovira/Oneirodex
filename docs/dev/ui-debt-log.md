...
|---|---|---|---|---|
| 2026-08-25 | UI UX-B7 | — | **Toasts on admin + library-add waits for scan end.** Member dismiss was already done. Admin SPA `showToast` gained the close button and polls `library_added`. Classic shells load `static/js/gt_toast.js` after notify and replace `$.notify`. Digest timer does not start while that library's scan is Running/Stopping; cancel still flushes. | `tests/test_gt_toast.py` · `tests/test_browse_path_status.py` digest hold · admin `toast.test.js` |
