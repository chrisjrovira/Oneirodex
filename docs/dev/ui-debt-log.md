...
|---|---|---|---|---|
| 2026-08-26 | Sec CSP + S2 | — | **Executable inline `<script>` extracted from Jinja; outbound fetches pin the checked DNS address.** Classic pages load `static/js/gt_*.js` (not theme copies). CSP stays report-only (`onclick=` + WebRetro eval). `safe_request` dials the IP that passed the SSRF check. | `tests/test_no_inline_scripts.py` · `tests/test_ssrf_hardening.py` pin cases · both in CI |
