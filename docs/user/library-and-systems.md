...
- Hovering a tile **grows it 25%** and drops its resting outline for an accent glow. With OS reduced-motion on you still get the size change — only the animated travel is suppressed.
- **Tile menu**: themed, every row the same shape, with **Report an issue** at the bottom (it opens the report form already filled in with the title). There is no "Play demo" item — GameTheca ships no demo feature.
- **Pagination**: page indicator on the left, First/Previous/Next/Last as one segmented control, per-page on the right.
- A cover that downloads badly can no longer distort its row — tiles are pinned to 3:4 and crop the overflow.
- **Preview** sits in the middle of a tile on hover: a larger popup with the cover, blurb and, at the bottom, **Available on** — every system in your libraries that holds this title, with a **Play** button per emulator core for each. Copies that cannot be launched are still listed, with the reason (missing firmware, catalog-only, no browser core). Backed by `GET /api/games/<uuid>/editions`, ACL-scoped like browse; titles are paired by name because `igdb_id` and `slug` are unique per row and so cannot be shared across systems.
