...
- **Signals** chips live **inside** the filter section (UPDATE · OUT/~ · NEW · RELEASE · LANG) and toggle the same browse params (`has_updates`, `freshness_behind`, `new_import`, `recent_release`, `needs_translation`); they persist with other library filters. Legacy `is_vr=1` URL params still apply if present.
