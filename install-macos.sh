...
#═══════════════════════════════════════════════
#   GameTheca macOS Installer v1.0
...
print_header() {
    clear
    echo -e "${CYAN}═══════════════════════════════════════════════${NC}"
    echo -e "${WHITE}    GameTheca macOS Installer v1.0${NC}"
    echo -e "${CYAN}═══════════════════════════════════════════════${NC}"
    echo
}

show_help() {
    echo "GameTheca macOS Installer"
    echo
    echo "USAGE:"
    ...
    echo "  you want available to a background GameTheca needs autofs instead —"
...
setup_postgresql() {
    ...
    # Homebrew's PostgreSQL trusts the local account, so GameTheca connects as
...
configure_application() {
    print_step "Configuring GameTheca..."
    ...
    # so pointing GameTheca at a NAS is a matter of naming the mount point.
...
# GameTheca Configuration - Generated by install-macos.sh $(date)
...
# Local HTTP — set both true once you put GameTheca behind HTTPS
...
validate_installation() {
    ...
    if python3 -c "from gametheca import create_app; create_app()" >/dev/null 2>&1; then
...
show_summary() {
    ...
    read -r -p "Start GameTheca now? [Y/n]: " start_now
    case "${start_now:-Y}" in
        [Yy]|[Yy][Ee][Ss])
            echo
            print_info "Starting GameTheca — open http://localhost:$CUSTOM_PORT"
