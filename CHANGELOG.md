...
- **Page-owning SPA loads use the shared status block.** Remaining admin Remote play / Libraries table / Integrations inventory / Images queue / Stock catalog, and member Systems / Cheats / Big Picture, use `PageStatus` instead of a bare “Loading…” paragraph. Trailers **Another one** keeps the current player up and floats `LoadingOverlay` (250ms) so the frame does not collapse.
