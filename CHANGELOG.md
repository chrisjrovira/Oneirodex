...
- **Tile preview shows GOG / Epic marks.** Browse still sends only Steam per cover. `GET /api/games/<uuid>/editions` now merges store URLs across copies the member can see, and the popup paints those marks the same way details does. A copy in a library they cannot see does not contribute a link.
