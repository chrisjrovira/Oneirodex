...
- Member SPA remaining `src/api/` wrappers sit on `createBrowserRequester` (same shape as admin `adminApi.ts`). FormData / fire-and-forget writes use `send` / `sendResult`. Preferences HTML and EventSource stay off this path.
