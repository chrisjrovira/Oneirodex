...
### Changed
- PC-cheat create and companion lifecycle POST adopt `@validate_body`. Missing `label` / non-list `records` are now `422 unprocessable`. Unknown cheat method, RetroArch-surface refusal, and missing companion token stay as they were after a well-formed body.
