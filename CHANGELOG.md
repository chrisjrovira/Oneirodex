...
### Changed
- Arr download and hardlink-preview adopt `@validate_body`. Missing `download_url` / `library_dest_dir` is now `422 unprocessable`. A well-formed preview body is still 403 when the hardlink pipeline is off (`tests/test_ops_followons.py`).
