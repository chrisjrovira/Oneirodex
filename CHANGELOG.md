...
- **Page-owning SPA loads use the shared status block.** Remaining admin Remote play / Libraries table / Integrations inventory / Images queue / Stock catalog, member Systems / Cheats / Big Picture / **Chat / Notifications / Activity** / **Library browse / FilterBar / Tokens**, and **Ops glance** use `PageStatus` instead of a bare “Loading…” paragraph. A failed first Ops poll no longer paints empty host panels. Trailers **Another one** keeps the current player up and floats `LoadingOverlay` (250ms) so the frame does not collapse. Tokens clipboard-unavailable copy stays an inline alert.
