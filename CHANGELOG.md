...
### Changed
- Wanted add/fulfill, hardlink preview/apply, game-server create, and admin malware-scan adopt `@validate_body`. Missing `game_uuid` / `source` / `dest` / `display_name` / `connect_string` / `path` are now `422 unprocessable`. Hardlink helper flags and child-cannot-create-servers still 403 after a well-formed body.
