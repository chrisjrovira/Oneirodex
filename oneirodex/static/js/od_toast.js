 ...
 * odShowAdminToast that run later are left alone if they already exist —
...
(function () {
  ...
  function visibleStackable(host) {
    return Array.prototype.filter.call(host.children, function (el) {
      return (
        el.className.indexOf('od-toast--out') === -1 &&
        (el.className.indexOf('od-toast--info') !== -1 ||
          el.className.indexOf('od-toast--success') !== -1)
      );
    });
  }
  ...
  function showToast(message, tone) {
    ...
    if (isStackableTone(safeTone)) {
      ...
      if (summary) {
        ...
        var summaryText = summary.querySelector('.od-toast__text');
        if (summaryText) {
