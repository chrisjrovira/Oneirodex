...
(function () {
    ...
    const form = document.getElementById('odLoadingIconForm');
    if (!form) {
        return;
    }
    const select = document.getElementById('odLoadingIconId');
    const preview = document.getElementById('odLoadingIconPreview');
    const loadStatus = document.getElementById('odLoadingIconStatus');
    ...
    function refreshPreview() {
        ...
        if (mode === 'lock' && id) {
            preview.appendChild(window.GtLoadingMotifs.buildNode(id, 'od-loading-motif--lg'));
            ...
            window.GtLoadingMotifs.CATALOGUE.forEach(function (motifId) {
                var node = window.GtLoadingMotifs.buildNode(
                    motifId, 'od-loading-motif--sm od-loading-motif--preview');
            ...
            if (window.odLoadingCatalogueSize > 6) {
                var note = document.createElement('p');
                note.className = 'od-themes-loading__note';
                note.textContent =
                    'Rotating across ' + window.odLoadingCatalogueSize
    ...
    .then(function (data) {
        ...
        window.odLoadingCatalogueSize = catalogue.length;
        if (select) {
            select.innerHTML = '';
