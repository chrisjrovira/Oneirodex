...
(function () {
  const statusEl = document.getElementById('od-emu-status');
  const listEl = document.getElementById('od-emu-profiles');
  ...
  function render(catalog, profiles) {
    ...
    listEl.innerHTML = platforms.map((platform) => {
      ...
      const options = ['<option value="">Default (first core)</option>']
        ...
        .join('');
      return `<div class="od-emu-row mb-2 d-flex gap-2 align-items-center flex-wrap">
    ...
    }).join('') + `<button type="button" class="btn btn-primary mt-2" id="od-emu-save">Save profiles</button>`;

    document.getElementById('od-emu-save')?.addEventListener('click', save);
  }
