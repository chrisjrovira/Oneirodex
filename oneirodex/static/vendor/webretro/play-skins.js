...
(function (global) {
  ...
  var NINTENDO = {
    ...
    WII: 1, N3DS: 1, SWITCH: 1, WII_U: 1, POKE_MINI: 1, GAME_WATCH: 1,
  };
  ...
  var PC = {
    ...
    ATARI_8BIT: 1, X68000: 1, PC_98: 1, BBC_MICRO: 1,
   ...
   * Handhelds (GB/GBA/NDS/etc.) already get a narrow --od-play-bezel-max
  ...
  var ASPECT_RATIOS = {
    ...
    GB: [10, 9], GBC: [10, 9], GBA: [3, 2], NDS: [2, 3], VB: [4, 3], POKE_MINI: [10, 9], GAME_WATCH: [10, 9],
    ...
    ATARI_ST: [4, 3], APPLE_II: [4, 3], ATARI_8BIT: [4, 3], X68000: [4, 3], PC_98: [4, 3], BBC_MICRO: [4, 3],
  ...
  // --od-play-scanline-opacity, which defaults to 0. Nothing ever set it, so
  // the effect shipped invisible. This wires it to the room.
  //
  // Mirrors oneirodex/utils/play_rooms.py (source of truth) and the member-app
  ...
  var ROOM_PLATFORMS = {
    wood_den_80s: 'ARCADIA ASTROCADE ATARI_2600 ATARI_5200 ATARI_7800 CHAF COLECO CREATIVISION GAME_WATCH GX4000 INTV NES O2EM PCE SEGA_MS SEGA_SG1000 STUDIO2 SUPERGRAFX VECTREX',
    ...
    desk: 'AMIGA AMIGA_CD32 APPLE_II ATARI_8BIT ATARI_ST BBC_MICRO CPC MAC MSX OTHER PCDOS PCWIN PC_98 VICE_X128 VICE_X64SC VICE_XPET VICE_XPLUS4 VICE_XVIC X68000 ZX_SPECTRUM'
  };
  ...
  var LCD_PLATFORMS = {
    ...
    LYNX: 1, NGP: 1, NGPC: 1, WS: 1, SUPERVISION: 1, ADVISION: 1, SEGA_GG: 1, POKE_MINI: 1, GAME_WATCH: 1
  };
  ...
  var STORAGE_PICTURE = 'od-play-picture';
    ...
    if (root) {
      ...
      if (next === 'crt') {
        root.style.setProperty(
          '--od-play-scanline-opacity',
          String(scanlineFor(platform, room))
        );
      } else {
        root.style.setProperty('--od-play-scanline-opacity', '0');
      }
    }
    try {
  ...
  function applyPlaySkin(platformId, coreId) {
    ...
    root.style.setProperty('--od-play-accent', meta.accent);
    root.style.setProperty('--od-platform-accent', meta.accent);
    ...
    root.style.setProperty('--od-play-aspect', aspectCss);
    body.style.setProperty('--od-play-aspect', aspectCss);
