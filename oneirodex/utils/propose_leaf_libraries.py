...
from oneirodex.utils.gamenames import LETTER_BUCKET_RE, should_skip_scan_dir
...
_ROM_FILE_EXTS = frozenset({
    ...
    'mgw', 'ssd', 'dsd', 'uef',
}) | {suffix.lstrip('.') for suffix in (PLATFORM_DUMP_SUFFIXES | ARCHIVE_EXTENSIONS)}
...
_PLATFORM_NAME_RULES: tuple[tuple[re.Pattern[str], str], ...] = (
    ...
    # Game & Watch before Game Boy so "Game Watch" never lands on GB.
    (re.compile(r'game\s*(?:(?:and|&)\s*)?watch|gamewatch|\bg\s*&\s*w\b', re.I), 'GAME_WATCH'),
    ...
    (re.compile(r'bbc\s*micro|\bbeeb\b', re.I), 'BBC_MICRO'),
)
