...
class ThemeManager:
    ...
    def get_installed_themes(self):
        ...
        if not os.path.isdir(self.theme_folder):
            return themes
        for theme_folder in sorted(os.listdir(self.theme_folder)):
            theme_path = os.path.join(self.theme_folder, theme_folder)
                ...
                if os.path.exists(json_path):
                    try:
                        with open(json_path, 'r') as json_file:
                            ...
                            themes.append({
                                # Folder slug is what preferences / theme_asset use
                                'folder': theme_folder,
                                'name': theme_data.get('name', theme_folder),
                        ...
                        print(f"Error reading theme {theme_folder}: {str(e)}")
        return themes
    ...
    def delete_themefile(self, theme_name):
        folder = secure_filename(theme_name)
        if folder in ('Default', 'default') or theme_name in ('Default', 'default'):
            raise ValueError("Cannot delete the default theme.")

        theme_path = os.path.join(self.theme_folder, folder)
        ...
        db.session.execute(
            ...
            .filter(UserPreference.theme.in_([theme_name, folder]))
