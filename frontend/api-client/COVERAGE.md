...
- Account `uploadAvatar` (multipart `POST /api/account/avatar`, currently in
  `@oneirodex/ui`'s `accountApi.ts` from PR-4 sub-wave d) still needs to move
  into `account.ts` here — tracked as the next step in this same PR.
