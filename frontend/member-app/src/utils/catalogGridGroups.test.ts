...
describe('groupCatalogGamesByGenre', () => {
  test('buckets by first genre and keeps first-seen order', () => {
    ...
    expect(sections[0].games.map((g: any) => g.uuid)).toEqual(['1', '3'])
    expect(sections[1].games.map((g: any) => g.uuid)).toEqual(['2'])
    expect(sections[2].games.map((g: any) => g.uuid)).toEqual(['4'])
  })
})
