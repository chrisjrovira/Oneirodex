 ...
 *
 * `color` is a `var(--od-brand-*)` reference, not a raw hex — the canonical
 * values live in `oneirodex/setup/default_theme/css/od-tokens.css`. The chip
 * sets `style={{ '--od-store-color': brand.color }}`, so the token resolves at
 * the use site in ExternalStoreLinks.css.
 */
const BRANDS = [
  {
    id: 'steam',
  ...
  },
  {
    ...
    label: 'PlayStation',
    color: 'var(--od-brand-playstation)',
...
function BrandIcon({ brand }: LooseProps) {
  if (brand.mask) {
    return (
      <span
        className="od-store-link__icon od-store-link__icon--mask"
        style={{ '--od-store-mask': `url("${brand.mask}")` } as CSSProperties}
        aria-hidden="true"
      />
    )
  }
        ...
        return (
          <li key={row.url} className="od-store-links__item">
            <a
              ...
              style={{ '--od-store-color': brand.color } as CSSProperties}
            >
