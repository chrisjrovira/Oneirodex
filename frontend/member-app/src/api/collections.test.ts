...
describe('member collections typed resource', () => {
  beforeEach(() => {
    ...
    global.fetch = vi.fn(() => jsonResponse({ ok: true })) as unknown as typeof global.fetch
  })
  ...
  test('fetchCollections GETs /api/collections over the browser transport', async () => {
    vi.mocked(global.fetch).mockReturnValue(jsonResponse({ collections: [] }))
  ...
  test('createCollection POSTs snake_case JSON and CSRF', async () => {
    vi.mocked(global.fetch).mockReturnValue(
      jsonResponse({ uuid: 'c1', name: 'Roguelites' }, { status: 201 }),
    )
    await expect(
  ...
  test('updateCollection PATCHes only provided fields as is_public', async () => {
    ...
    expect(vi.mocked(global.fetch).mock.calls[0][0]).toBe('/api/collections/abc-123')
    expect(JSON.parse(init!.body as string)).toEqual({ name: 'Cozy remixed', is_public: false })
  })
  ...
  test('item add/remove/reorder hit the typed collection item paths', async () => {
    ...
    const urls = vi.mocked(global.fetch).mock.calls.map((call: any) => [call[0], call[1]?.method])
    expect(urls).toEqual([
    ...
    expect(JSON.parse(vi.mocked(global.fetch).mock.calls[2][1]!.body as string)).toEqual({
      game_uuids: ['g9', 'g2'],
    })
  })
  ...
  test('deleteCollection and fetchCollection keep labels on envelope errors', async () => {
    ...
    vi.mocked(global.fetch).mockReturnValueOnce(
      jsonResponse({ error: 'missing' }, { ok: false, status: 404 }),
    )
  ...
  test('searchGames stays on /api/search and slices the array', async () => {
    ...
    vi.mocked(global.fetch).mockReturnValue(jsonResponse(rows))
    await expect(searchGames('  cel  ', { limit: 2 })).resolves.toEqual(rows.slice(0, 2))
    expect(vi.mocked(global.fetch).mock.calls[0][0]).toBe('/api/search?query=cel')
