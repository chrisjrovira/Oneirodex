...
describe('member browser-transport verbs', () => {
  ...
  test('patchJson and deleteJson keep method and CSRF', async () => {
    ...
    const patchCall = vi
      .mocked(global.fetch)
      .mock.calls.find((call: any) => call[1]?.method === 'PATCH')
    const deleteCall = vi
      .mocked(global.fetch)
      .mock.calls.find((call: any) => call[1]?.method === 'DELETE')
    expect(patchCall![0]).toBe('/api/requests/7')
    expect(new Headers((patchCall as any[])[1].headers).get('X-CSRFToken')).toBe('test-csrf-token')
    expect(deleteCall![0]).toBe('/api/requests/7')
    expect(new Headers((deleteCall as any[])[1].headers).get('X-CSRFToken')).toBe('test-csrf-token')
  })
