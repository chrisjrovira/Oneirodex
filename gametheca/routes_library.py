...
from gametheca.utils.functions import format_size, get_library_count, get_games_count
from gametheca.utils.local_metadata import has_local_metadata, has_local_images
from gametheca.utils.auth import admin_required
from gametheca.forms import CsrfForm, CsrfProtectForm
...
def get_games(page=1, per_page=20, sort_by='name', sort_order='asc', **filters):
    ...
    if 'library_uuid' in filters and filters['library_uuid']:
        if not user_can_access_library(current_user, filters['library_uuid']):
            return [], 0, 0, page
    ...
    for game in games:
        ...
        if cover_image and cover_image.url:
            if cover_image.url.startswith(('http://', 'https://', '/static/')):
                cover_url = cover_image.url
            else:
                cover_url = url_for(
                    'static',
                    filename=f"library/images/{cover_image.url.lstrip('/')}",
                )
        else:
