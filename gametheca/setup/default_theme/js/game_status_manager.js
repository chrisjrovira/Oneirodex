...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateButtonAppearance = (button, status) => {
        ...
        let mark = button.querySelector('.gt-status-dot, .gt-spinner, i');
        if (!mark) {
            mark = document.createElement('span');
            mark.className = 'gt-status-dot';
            button.prepend(mark);
        }
        mark.className = 'gt-status-dot';
        mark.textContent = '';
    ...
    const setGameStatus = async (button, gameUuid, newStatus) => {
        ...
        try {
            mark.className = 'gt-spinner gt-spinner--sm';
    ...
    const showSuccessAnimation = async (button) => {
        return new Promise((resolve) => {
            const mark = button.querySelector('.gt-status-dot, .gt-spinner, i');
            if (!mark) {
                resolve();
                return;
            }
