document.addEventListener('DOMContentLoaded', function() {
    ...
    async function ensureCustomIgdbId() {
        const current = parseInt(igdbIdInput.value, 10);
        if (!Number.isNaN(current) && current >= 2000000420) {
            return current;
        }
    ...
    function expandGameDetails() {
        const gameDetailsCollapse = document.querySelector('#gameDetails');
        if (gameDetailsCollapse) {
            new bootstrap.Collapse(gameDetailsCollapse, { show: true });
        }
    }

    function searchExternalSource(source) {
        const gameName = nameInput.value.trim();
        const resultsContainer = document.querySelector('#search-results');
        if (!gameName) {
            resultsContainer.textContent = 'Enter a game name first';
            return;
        }
        resultsContainer.textContent = `Searching ${source.toUpperCase()}…`;
        fetch(`/api/search_metadata?source=${encodeURIComponent(source)}&name=${encodeURIComponent(gameName)}`)
            .then(response => response.json())
            .then(data => {
                resultsContainer.innerHTML = '';
                if (data.error) {
                ...
                data.results.forEach(hit => {
                    ...
                    img.src = hit.cover_url || '/static/newstyle/nocoverfound.png';
                    resultItem.appendChild(img);
                    const label = document.createElement('span');
                    label.textContent = `[${source.toUpperCase()}] ${hit.name || 'Unknown'}`;
                    resultItem.appendChild(label);
                    resultItem.addEventListener('click', async function() {
                        expandGameDetails();
                        await ensureCustomIgdbId();
                        nameInput.value = hit.name || nameInput.value;
                        if (hit.summary) {
                            document.querySelector('#summary').value = hit.summary;
                        }
                        if (hit.url) {
                        ...
                        checkFieldsAndToggleSubmit();
                        resultsContainer.innerHTML = '';
                        $.notify(`Applied ${source.toUpperCase()} match (custom IGDB ID). Save to keep.`, {
                            className: 'success',
                            position: 'top center'
                        });
                    });
            ...
            .catch(error => {
                console.error(error);
                resultsContainer.textContent = `Search failed: ${error}`;
            });
    }
    ...
    // --- Link to Existing Library Game ---
    const existingGameSearchInput = document.querySelector('#existing-game-search');
    const existingGameResults = document.querySelector('#existing-game-results');
    const existingGameSelection = document.querySelector('#existing-game-selection');
    const existingGameSelectionText = document.querySelector('#existing-game-selection-text');
    const linkExistingGameBtn = document.querySelector('#link-existing-game-btn');
    const linkExistingGameForm = document.querySelector('#link-existing-game-form');
    const linkExistingGameUuidInput = document.querySelector('#link-existing-game-uuid');
    const linkExistingFullDiskPathInput = document.querySelector('#link-existing-full-disk-path');

    if (existingGameSearchInput && existingGameResults && linkExistingGameBtn && linkExistingGameForm) {
        let selectedExistingGame = null;
        let existingGameSearchDebounce = null;

        function clearExistingGameSelection() {
            selectedExistingGame = null;
            linkExistingGameBtn.disabled = true;
            if (existingGameSelection) {
                existingGameSelection.style.display = 'none';
            }
        }

        function renderExistingGameResults(games) {
            existingGameResults.innerHTML = '';
            if (!games || games.length === 0) {
                existingGameResults.textContent = 'No matching games found';
                return;
            }

            games.forEach(function(game) {
                const item = document.createElement('div');
                item.className = 'search-result-item existing-game-result-item';

                const nameSpan = document.createElement('span');
                nameSpan.className = 'existing-game-result-name';
                nameSpan.textContent = game.name;

                const pathSpan = document.createElement('span');
                pathSpan.className = 'existing-game-result-path';
                pathSpan.textContent = game.full_disk_path || '(no path set)';

                item.appendChild(nameSpan);
                item.appendChild(pathSpan);

                item.addEventListener('click', function() {
                    selectedExistingGame = game;

                    existingGameResults.querySelectorAll('.existing-game-result-item').forEach(function(el) {
                        el.classList.remove('selected');
                    });
                    item.classList.add('selected');

                    if (existingGameSelectionText) {
                        existingGameSelectionText.textContent = `Link "${game.name}" to this folder?`;
                    }
                    if (existingGameSelection) {
                        existingGameSelection.style.display = 'flex';
                    }
                    linkExistingGameBtn.disabled = false;
                });

                existingGameResults.appendChild(item);
            });
        }

        function searchExistingGames(term) {
            if (!term || term.trim().length < 2) {
                existingGameResults.innerHTML = '';
                clearExistingGameSelection();
                return;
            }

            fetch(`/api/admin/games_search?q=${encodeURIComponent(term.trim())}`)
                .then(function(response) { return response.json(); })
                .then(function(data) {
                    if (Array.isArray(data)) {
                        renderExistingGameResults(data);
                    } else {
                        existingGameResults.textContent = data.error || 'Search failed';
                    }
                })
                .catch(function(error) {
                    console.error('Error searching existing games:', error);
                    existingGameResults.textContent = 'Error searching existing games';
                });
        }

        existingGameSearchInput.addEventListener('input', function() {
            clearExistingGameSelection();
            clearTimeout(existingGameSearchDebounce);
            const term = this.value;
            existingGameSearchDebounce = setTimeout(function() {
                searchExistingGames(term);
            }, 300);
        });

        linkExistingGameBtn.addEventListener('click', function() {
            if (!selectedExistingGame) {
                return;
            }
            linkExistingGameUuidInput.value = selectedExistingGame.uuid;
            linkExistingFullDiskPathInput.value = fullPathInput.value;
            linkExistingGameBtn.disabled = true;
            linkExistingGameForm.submit();
        });
    }
