...
(function ensureAdminToast() {
  if (typeof window.gtShowAdminToast === 'function') return;
  window.gtShowAdminToast = function (message, variant) {
    if (!message) return;
...
document.addEventListener('DOMContentLoaded', function () {
  const panel = document.getElementById('gtLibrariesPanel');
  if (!panel) return;
  ...
  /** Enum name → label; mirrors gametheca.platform.LibraryPlatform for batch edit. */
  ...
  function notify(message, tone) {
    if (typeof window.gtShowAdminToast === 'function') {
      window.gtShowAdminToast(message, tone);
      return;
    }
  ...
  function selectedRows() {
    return Array.from(document.querySelectorAll('.gt-library-row-check:checked')).map((el) => ({
  ...
  function syncSelectionUi() {
    ...
    const n = rows.length;
    const bar = document.getElementById('gtLibrariesBatchBar');
    const countEls = [
      document.getElementById('gtLibrariesSelectionCount'),
      document.getElementById('gtLibrariesBatchCount'),
    ];
    countEls.forEach((el) => {
      if (el) el.textContent = n === 1 ? '1 selected' : `${n} selected`;
    });
    if (bar) bar.hidden = n === 0;
    const selectAll = document.getElementById('gtLibrariesSelectAll');
    const checks = document.querySelectorAll('.gt-library-row-check');
  ...
  function clearSelection() {
    document.querySelectorAll('.gt-library-row-check').forEach((el) => {
      el.checked = false;
    });
    syncSelectionUi();
  }
  ...
  const clearBtn = document.getElementById('gtLibrariesBatchClear');
  ...
  const batchDeleteBtn = document.getElementById('gtLibrariesBatchDelete');
  if (batchDeleteBtn) {
  ...
  const batchEditBtn = document.getElementById('gtLibrariesBatchEdit');
  if (batchEditBtn) {
  ...
  const batchScanBtn = document.getElementById('gtLibrariesBatchScan');
  if (batchScanBtn) {
