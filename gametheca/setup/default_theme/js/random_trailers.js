...
let playedSeconds = 0;
let isPlaying = false;
let currentTrailerGameUuid = null;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (isAttractMode) {
        ...
        const openBigPictureBtn = document.getElementById('open-big-picture-btn');
        if (openBigPictureBtn) {
            openBigPictureBtn.style.display = 'inline-flex';
            openBigPictureBtn.addEventListener('click', openCurrentInBigPicture);
        }
    ...
    function openCurrentInBigPicture() {
        const uuid = currentTrailerGameUuid;
        if (uuid) {
            window.location.href = `/big-picture?game=${encodeURIComponent(uuid)}`;
        } else {
            window.location.href = '/big-picture';
        }
    }
    ...
    function displayTrailer(data) {
        ...
        currentTrailerGameUuid = data.game_uuid || null;
