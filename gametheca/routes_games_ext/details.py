...
from gametheca.utils.functions import format_size, sanitize_string_input, get_url_icon
...
def game_details(game_uuid):
    ...
    if game and not user_can_access_game(current_user, game):
        log_system_event(
    ...
    if game:
        ...
        if current_user_id:
            progress = db.session.execute(
                select(UserGameProgress).filter_by(
